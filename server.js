...
wss.on('connection', (ws) => {
  ...
  ws.on('message', (buf) => {
    ...
    if (m.t === 'create') {
      leaveRoom();
      const code = makeCode();
      room = { code, players: new Map(), hostId: id, started: false };
      room.players.set(id, { ws, name: m.name || `Jugador ${id}` });
      rooms.set(code, room);
      ...
      // Estado del jugador: retransmitir al resto de la sala (presencia).
